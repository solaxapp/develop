...
import {programIds, SWAP_HOST_FEE_ADDRESS, WRAPPED_SOL_MINT} from "../../helper/ids";
import {AccountLayout, MintLayout, u64} from "@solana/spl-token";
import {useConnection, useWallet} from "@solana/wallet-adapter-react";
import {useNotify} from "../Notifications";
import {chunks} from "../../helper/token";
import {useAllPoolsContext} from "../PoolsProvider";
...
export function AccountsProvider({children = null, network}) {
    const {connection} = useConnection();
    const {wallet, connected, publicKey} = useWallet();
    const [tokenAccounts, setTokenAccounts] = useState([]);
    const [userAccounts, setUserAccounts] = useState([]);
    const {nativeAccount} = UseNativeAccount();
    const {pools} = useAllPoolsContext();
    ...
    useEffect(() => {
        console.log("USE EFFECT AccountsProvider, 1")
        setUserAccounts([wrapNativeAccount(publicKey, nativeAccount), ...tokenAccounts,]
            .filter((a) => a !== undefined));
    }, [nativeAccount, publicKey, tokenAccounts]);

    useEffect(() => {
        console.log("USE EFFECT AccountsProvider, 2")
...
export function useMint(id) {
    ...
    useEffect(() => {
        ...
        cache.queryMint(connection, id).then(setMint).catch((err) => notify("error", err.message))
